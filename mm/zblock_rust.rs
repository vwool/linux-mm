...
fn alloc_block(pool: &ZblockPool, block_type: usize, gfp: Flags, nid: NumaNode) -> Result<usize> {
    let layout =
        Layout::from_size_align(PAGE_SIZE * pool.block_desc(block_type).n_pages, PAGE_SIZE);
    match layout {
        Err(_) => Err(EINVAL),
        Ok(l) => {
            let ptr = Vmalloc::alloc(l, gfp, nid)?;
            let block: *mut ZblockBlock = ptr.as_ptr().cast::<ZblockBlock>();
            // SAFETY: block is a non-null and valid pointer because it came from Vmalloc
            unsafe {
            ...
            inner.block_list.push_front(unsafe { ListArc::from_raw(block) });
...
impl Zpool for ZblockRust {
    ...
    fn create(_name: *const u8, gfp: Flags) -> Result<KBox<ZblockPool>> {
    ...
    fn malloc(the_pool: &mut ZblockPool, size: usize, gfp: Flags, nid: NumaNode) -> Result<usize> {
        ...
        match result {
            None => alloc_block(the_pool, block_type, gfp, nid),
    ...
    fn free(the_pool: &ZblockPool, handle: usize) {
        ...
        // SlotInfo bitops should have preemption disabled, so do it here
        ...
        match prev_free_slots {
            ...
            0 => {
                ...
                // * this ZblockBlock doesn't have a ListArc reference
                // * ZblockBlock doesn't track ListArc
                inner.block_list.push_back(unsafe { ListArc::from_raw(block) });
            }
            _ => {}
        }
    }

    fn read_begin(the_pool: &ZblockPool, handle: usize) -> *mut c_void {
        let (block, block_type, slot) = handle_to_metadata(handle);
        let handle_mem = (block as usize) + ZBLOCK_HEADER_SIZE!()
            + (slot as usize) * the_pool.block_desc(block_type).slot_size;

        handle_mem as *mut c_void
    }
    ...
    fn write(the_pool: &ZblockPool, handle: usize, handle_mem: *mut c_void, mem_len: usize) {
        ...
        let map_addr = (block as usize)
            + ZBLOCK_HEADER_SIZE!()
            + (slot as usize) * the_pool.block_desc(block_type).slot_size;
        // SAFETY: handle_mem is guaranteed to be a valid pointer by zpool, map_addr points to
        // an area within block
        unsafe {
            copy_nonoverlapping(handle_mem, map_addr as *mut c_void, mem_len);
        }
    }
...
impl kernel::Module for ZblockRust {
    fn init(_module: &'static ThisModule) -> Result<Self> {
        let _ = ZPOOL_DRIVER.register(RUSTY_BLOCK.get_name(), _module);
        Ok(RUSTY_BLOCK)
    }
}
