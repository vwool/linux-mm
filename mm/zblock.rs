...
impl ZblockPool {
    fn new(page_size: usize, name: CString, gfp: Flags) -> Result<Self> {
...
fn cache_find_block(list: &BlockList, block_desc: &BlockDesc) -> Option<(*const ZblockBlock, u16)> {
    ...
    if let Some(next) = cursor.peek_next() {
        ...
        if next.free_slots.fetch_sub(1, Ordering::Relaxed) == 1 {
...
struct ZblockCreator;

impl ZallocCreator for ZblockCreator {
    fn create_pool(&self, name: CString, gfp: Flags) -> Result<KBox<dyn ZallocDriver>> {
        let mut pool = ZblockPool::new(PAGE_SIZE, name, gfp)?;
    ...
    fn borrow_pool(&self, ptr: *mut c_void) -> &'static dyn ZallocDriver {
        ...
        unsafe { &*ptr.cast::<ZblockPool>() }
    }

    fn from_raw(&self, ptr: *mut c_void) -> KBox<dyn ZallocDriver> {
        // SAFETY: `ptr` is a pointer to the pool previously allocated by `create_pool`
        unsafe { KBox::from_raw(ptr.cast::<ZblockPool>()) }
    }
}
...
impl ZallocDriver for ZblockPool {
    fn malloc(&self, size: usize, gfp: Flags, nid: NumaNode) -> Result<ZallocHandle> {
        ...
        let cursor = self.tree.cursor_lower_bound(&size);
        ...
        let list = &self.block_lists[block_type];
        match cache_find_block(list, &self.block_descs[block_type]) {
            None => self.alloc_block(block_type, gfp, nid),
    ...
    unsafe fn free(&self, handle: ZallocHandle) {
        ...
        the_block.slot_info.clear(slot); // XXX?
...
kernel::DeclareZallocBackend!(ZblockCreator);
