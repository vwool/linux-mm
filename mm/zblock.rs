...
use kernel::alloc::{Allocator, Flags, flags, KVec, NumaNode};
...
impl SlotInfo {
    ...
    fn set(&mut self, slot: u16) {
        let rem = slot & 0x3F;
        loop {
            let val = self.info[(slot >> 6) as usize].load(Ordering::Acquire);
    ...
    fn find_and_set(&mut self, max_slots: u16) -> u16 {
        ...
        loop {
            for i in 0..i_max {
                ...
                loop {
                    let val = self.info[i as usize].load(Ordering::Acquire);
...
#[inline]
fn metadata_to_handle(block: *const ZblockBlock, block_type: usize, slot: u16) -> ZpoolHandle {
    let b: usize = block.cast::<usize>() as usize;
    ZpoolHandle::from_raw(b + (block_type << SLOT_BITS) + (slot as usize))
}

#[inline]
fn handle_to_metadata(handle: ZpoolHandle) -> (*mut ZblockBlock, usize, u16) {
    let h = handle.as_raw();
...
impl ZpoolDriver for ZblockRust {
    ...
    fn create(_name: CString, gfp: Flags) -> Result<KBox<ZblockPool>> {
    ...
    fn malloc(pool: &ZblockPool, size: usize, gfp: Flags, nid: NumaNode) -> Result<ZpoolHandle> {
        if size == 0 || size > PAGE_SIZE {
            return Err(EINVAL);
        }

        let cursor = pool.tree.cursor_lower_bound(&size);
        ...
        let result = cache_find_block(list, &pool.block_descs[block_type]);
        match result {
            None => pool.alloc_block(block_type, gfp, nid),
            Some((block, slot)) => Ok(metadata_to_handle(block, block_type, slot)),
        }
    }
    unsafe fn free(the_pool: &ZblockPool, handle: ZpoolHandle) {
    ...
    unsafe fn read_begin(the_pool: &ZblockPool, handle: ZpoolHandle) -> NonNull<u8> {
        let (block, block_type, slot) = handle_to_metadata(handle);
    ...
    unsafe fn read_end(_pool: &ZblockPool, _handle: ZpoolHandle, _handle_mem: NonNull<u8>) {}
    ...
    unsafe fn write(
        ...
        let map_addr = (block as usize)
            + ZBLOCK_HEADER_SIZE!()
            + (slot as usize) * pool.block_desc(block_type).slot_size;
...
kernel::DECLARE_ZPOOL!(ZblockRust);
