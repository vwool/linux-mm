					...
					unsigned int nid)
{
	...
	struct zblock_block *block = kmem_cache_zalloc(pool->block_header_cache, gfp);

	if (!block)
		goto out_cache;

	block->page_block = __vmalloc_node(PAGE_SIZE * num_pages, PAGE_SIZE, gfp, nid, NULL);
	if (!block->page_block)
		goto out_alloc;
...
static void *zblock_map(struct zblock_pool *pool, unsigned long handle)
{
	...
	unsigned long offs = slot * block_desc[block->block_type].slot_size;

	return block->page_block + offs;
}
			 ...
			 void *handle_mem, size_t mem_len)
{
	...
	memcpy(block->page_block + offs, handle_mem, mem_len);
}
