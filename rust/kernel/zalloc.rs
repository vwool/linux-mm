...
/// }
///
/// struct MyZallocCreator;
///
/// impl ZallocCreator for MyZallocCreator {
///     fn create_pool(&self, name: CString, gfp: Flags) -> Result<KBox<dyn ZallocDriver>> {
///         let pool = MyZalloc { name, bytes_used: AtomicU64::new(0) };
...
///         unsafe { &*ptr.cast::<MyZalloc>() }
///     }
///
///     fn from_raw(&self, ptr: *mut c_void) -> KBox<dyn ZallocDriver> {
///         // SAFETY: `ptr` is a pointer to the pool previously allocated by `create_pool`
///         unsafe { KBox::from_raw(ptr.cast::<MyZalloc>()) }
///     }
/// }
///
...
/// // kernel::DeclareZallocBackend!(MyZallocCreator);
/// ```
///
...
macro_rules! DeclareZallocBackend {
    ($t: ty) => {
        ...
        #[no_mangle]
        static __default_zalloc_backend: &'static dyn ZallocCreator = <$t>::new();
    };
}
